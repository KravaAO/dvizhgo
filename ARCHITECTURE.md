...
Платформа не обмежується школою, тому в новій room-моделі не використовуємо
терміни `teacher` і `student` у назвах таблиць, API, сервісів або UI.

- **Room host** — людина, що створює кімнату; вона отримує право керувати
  кімнатою та запускати activity. У шкільному сценарії це може бути викладач,
  але це не системна роль «викладач».
- **Participant** — людина, що приєдналася до кімнати за кодом. У шкільному
  сценарії це може бути учень, але модель не обмежується учнями.
- **Room** — ізольований live-контекст для participants, activities, результатів
  і подій.

Майбутня схема використовує `rooms`, `room_hosts` (або host session) та
`participants`. Наявні `students` і глобальні roulette/results є legacy-шаром,
який буде поступово перенесений у межі конкретної room без зламу поточного quiz.
